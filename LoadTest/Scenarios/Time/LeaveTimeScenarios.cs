...
namespace DigitalOffice.LoadTesting.Scenarios.Time
{
    public class LeaveTimeScenarios : BaseScenarioCreator
    {
        ...
        private Scenario Find(FindLeaveTimesFilter filter, HttpStatusCode expected)
        {
            var correct = Step.Create("find", async context =>
                CreateResponse(await _leaveTimeController.Find(filter), expected));
        ...
        public override void Run()
        {
            Guid? leaveTimeId = JsonConvert
                .DeserializeObject<FindResultResponse<LeaveTimeResponse>>(
                    _leaveTimeController.Find(new FindLeaveTimesFilter { SkipCount = 0, TakeCount = 1 }).Result.Content.ReadAsStringAsync().Result)
                .Body
                .FirstOrDefault()
                ?.LeaveTime.Id;
