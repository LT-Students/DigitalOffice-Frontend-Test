...
namespace DigitalOffice.LoadTesting.Services.Time
{
  public class WorkTimeDayJobController
  {
    ...
    public Task<HttpResponseMessage> Create(CreateWorkTimeDayJobRequest request)
    {
    ...
    public Task<HttpResponseMessage> Edit(Guid workTimeDayJobId, List<(string property, string newValue)> changes)
    {
      var httpContent = new StringContent(
      ...
      return _httpClient.PatchAsync(CreateEditWorkTimeDayJobRequest(workTimeDayJobId), httpContent);
    }
  }
}
