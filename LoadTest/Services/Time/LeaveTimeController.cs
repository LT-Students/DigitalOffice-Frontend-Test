...
namespace DigitalOffice.LoadTesting.Services.Time
{
  public class LeaveTimeController
  {
    ...
    public Task<HttpResponseMessage> Add(CreateLeaveTimeRequest request)
    {
    ...
    public Task<HttpResponseMessage> Find(FindLeaveTimesFilter filter)
    {
      return _httpClient.GetAsync(CreateFindLeaveTimeRequest(filter));
    }

    public Task<HttpResponseMessage> Edit(Guid leaveTimeId, List<(string property, string newValue)> changes)
    {
      var httpContent = new StringContent(
      ...
      return _httpClient.PatchAsync(CreateEditLeaveTimeRequest(leaveTimeId), httpContent);
    }
  }
}
